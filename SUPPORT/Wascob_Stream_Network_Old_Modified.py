...
## WASCOB Stream Network will run on an aggregated DEM of 3m or 10 ft resolution if input is finer.
...
def determineOverlap(culvertLayer):
    # This function will evaluate the input culverts vs the updated project culverts layer and create
    # messages about the culverts that will be used.
    ...
    try:
        ...
        if numOfCulvertsWithinAOI > 0:
            if numOfCulverts == numOfCulvertsWithinAOI:
                ...
                AddMsgAndPrint("\tCulverts within the area of interest will be used to hydro enforce the DEM!",0)
                return True
        else:
...
try:
    ...
    if not arcpy.Exists(watershedGDB_path) or not arcpy.Exists(DEM_aoi):
        ...
        AddMsgAndPrint("Please run the Define Area of Interest tool!",2)
        sys.exit(0)
            ...
            if not arcpy.Describe(burnCulverts).CatalogPath == culverts:
                ...
                if arcpy.Exists(culverts):
                    try:
                        ...
                        AddMsgAndPrint("\nRemoved previous culverts project data.",0)
                    except:
                        print_exception()
                # Create new or re-create culverts layer from new input by clipping to AOI, in case input dataset is very large
                arcpy.Clip_analysis(burnCulverts, projectAOI_path, culverts)
                AddMsgAndPrint("\nSuccessfully created a new culverts feature class!",0)           
            # paths are the same therefore input was from within FGDB
            else:
                AddMsgAndPrint("\nUsing existing culverts feature project data!",0)
                reuseCulverts = True
